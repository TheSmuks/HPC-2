...
int master(int argc, char **argv) {
    ...
    int *next_chunk_for_worker = NULL;
    ...
    next_chunk_for_worker = (int *)calloc(worker_count + 1, sizeof(int));
    for (int r = 1; r <= worker_count; r++) next_chunk_for_worker[r] = r - 1;
    ...
    while (terminated_workers < worker_count) {
        ...
        if(status.MPI_TAG == WORK_TAG) {
            ...
            mpi_time_accum += MPI_Wtime() - start;
            if (next_chunk_for_worker[worker_rank] < valid_chunk_count) {
                int c = next_chunk_for_worker[worker_rank];
                ChunkInfo *info = &all_chunks[c];

                start = MPI_Wtime();
                send_chunk_info(info, worker_rank, WORK_TAG);
                MPI_Send(&chunk_byte_offsets[c], 1, MPI_LONG,
                         worker_rank, WORK_TAG, MPI_COMM_WORLD);
                mpi_time_accum += MPI_Wtime() - start;
                next_chunk_for_worker[worker_rank] += worker_count;
            } else {
                start = MPI_Wtime();
